...
// 1. Consulta principal (Asegúrate de que 'precio2' y 'Minimo' existan en tu tabla)
$res = $conn->query("SELECT idproducto, descripcion, precio2, disponible, Minimo, foto, Tipo,Venta FROM productos WHERE disponible >= 0 and precio2 > 0 ORDER BY Tipo ASC, descripcion ASC");
...
<head>
    ...
    <title>VenyPaga | Mayorista</title>
...
<body>
                    ...
                    <?php foreach($items as $prod): ?>
                        <div class="producto-item bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col transition hover:shadow-md" 
                            ...
                            <div class="relative aspect-square overflow-hidden bg-gray-50">
                                ...
                                <div class="absolute bottom-2 left-2 bg-indigo-600 text-white px-2 py-1 rounded-lg font-bold text-[10px] shadow-lg">
                                    MIN: <?php echo $prod['Minimo']; ?> UND
                                </div>
                                <?php endif; ?>
                            ...
                            <div class="p-5 flex-grow flex flex-col">
                                ...
                                <div class="mt-4 flex flex-col">
                                    <span class="text-xl font-black text-blue-600">$<?php echo number_format($prod['precio2'], 2); ?></span>
    ...
    <script>
        ...
        function verNota() {
            ...
            if (nota.length === 0) {
                ...
                nota.forEach((p, index) => {
                    const subtotal = parseFloat(p.precio2) * p.cantidad;
                    total += subtotal;
                    lista.innerHTML += `
                        <div class="flex items-center justify-between group">
                            <div class="flex-grow">
                                <p class="font-black text-gray-800 text-sm leading-tight uppercase">${p.descripcion}</p>
                                <p class="text-blue-600 font-black text-xs mt-1">${p.cantidad} UND x $${parseFloat(p.precio2).toFixed(2)}</p>
                            </div>
        ...
        function enviarWhatsApp() {
            ...
            const total = nota.reduce((s, p) => s + (parseFloat(p.precio2) * p.cantidad), 0);
            ...
            .then(data => {
                if(data.ok) {
                    ...
                    nota.forEach(p => {
                        ...
                        msg += `Subtotal: $ ${(parseFloat(p.precio2) * p.cantidad).toLocaleString('es-ES')}\n\n`;
                    });
                    msg += `━━━━━━━━━━━━━━━━━━\n`;
